...
async function getProxyList() {
  const proxyBankUrl = "https://raw.githubusercontent.com/Novan6966/worker/main/output.txt"; // Tetap menggunakan proxyBankUrl
  try {
...
<body class="bg-light">
    ...
    <a href="https://t.me/novanthekillz" target="_blank" class="text-decoration-none">
        NOVAN TUNNEL
    </a>
</h1>
<div class="card-body">
    ...
    for (let i = startIndex; i < startIndex + 50; i++) {
      ...
      const Status = `https://ipcf.vlessipcf.us.kg/key=nana/ip=${proxyIP}:${proxyPort}`;
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/") {
        ...
        const proxy_Url = url.searchParams.get("proxy-list"); // Expect the proxy list from query params
        let proxyList = (await getProxyList(proxy_Url)).filter((proxy) => {
